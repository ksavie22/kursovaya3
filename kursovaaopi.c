...
int main() {
    ...
    do {
        ...
        case 3:
            ...
            double x_ext, f_ext;
            if (find_extremum(a, b, h, &x_ext, &f_ext)) {
...
int find_extremum(double a, double b, double h, double* x_ext, double* f_ext) {
    int found = 0;
    double fmin = INFINITY, fmax = -INFINITY, xmin = 0, xmax = 0;
...
void create_table(double a, double b, int points) {
    ...
    for (int i = 0; i < points; i++) {
        ...
        if (is_defined(x)) {
            ...
            save_calc_result("table_point", x, 0, 0, val, 0);
        }
        else {
            printf("| %12.4lf | %16s |\n", x, "не определена");
...
void save_calc_result(const char* type, double param1, double param2, double param3, double result1, double result2) {
    ...
    if (strcmp(type, "f(x)") == 0 || strcmp(type, "derivative") == 0 || strcmp(type, "table_point") == 0) {
